...
**The One Source Rule (drift guard).** The accent value exists twice today – `--pop-pink` in `@layer tokens` and a hardcoded `0xFF00A9` inside `<kinetic-background>`. If the pink ever changes, the stardust won't follow. The component's source already notes a future `data-color` attribute as the fix; until then, the token is canonical and the hardcode is drift to reconcile.
...
**Layered glass over living stardust.** Two layers, in order: the `<kinetic-background>` component animates behind everything – the site's living depth, present on the pages that load it (index, beyond-the-word-list, codepen-converter). It is a fixed, full-viewport transparent canvas holding 2000 points in a cubic volume, rendered in Neon Pop Pink with additive blending at 0.8 opacity: the points are tiny square specks, mostly 1–3 px on screen and roughly 35 px apart, so the field reads as a sparse starfield, not a cloud – a slow snow-globe of pink dust. The whole cloud rotates imperceptibly (a full turn every ~5 minutes, reading as ambient drift), and the camera eases toward the cursor (about a third of a second of lag), so near specks visibly swim against far ones – the parallax is the dominant motion. Under `prefers-reduced-motion` no renderer is created at all: the canvas stays blank and transparent, and the violet-black night shows through. Glass cards float above it, letting the stardust show through (backdrop blur 12px, a subtle white gradient at 5% fading to transparent, a 1px translucent border). Neon glow is the accent's own light: the CTA glows pink at rest and harder on hover, the hero title carries a text-shadow glow. There are no drop shadows in the system – a neon lab has nothing that casts gray.
...
The living stardust behind the pages that load it – and the depth model itself, so its rules are the system's rules. Precisely: a fixed, full-viewport transparent canvas (shadow DOM, z-index 0, pointer-events none), 2000 points in a 1000-unit cube, colored Neon Pop Pink, size 1.5 world-units, additive blending at 0.8 opacity. The points are square specks (no round-particle shader), mostly 1–3 px on screen at ~35 px spacing – a sparse starfield, deliberately. Two motions: the cloud rotates rigidly around its Y axis at about 1.15° per second (imperceptible as rotation – ambient drift), and the camera eases toward the cursor (±50 units, ~330 ms lag), giving a strong near/far parallax. The additive blending is the mechanism of "the accent's own light": where points nearly overlap they saturate toward hot magenta-white. The color is bound to Neon Pop Pink by hardcode (`0xFF00A9`) rather than token – a known drift risk (see the One Source Rule note). Under `prefers-reduced-motion` the renderer is never created; the canvas stays blank, a static, calm night.

Known gaps, all planned or documented: it lacks `aria-hidden` (decorative by contract; the content plan adds it), it does not pause on hidden tabs (browsers already throttle rAF in background tabs, so this is hygiene more than a win), it has no `setPixelRatio` cap (the canvas renders at CSS-pixel resolution – soft on Retina, arguably on-brand for a glow, currently unstated), and the renderer/geometry are not disposed on disconnect (latent – the element is never re-added in this static MPA). Tuning values – particle count, rotation rate, parallax factor – live in the code, not here.
