...
function unicodeRangeToString(rangeStr) {
  ...
  for (const part of parts) {
    if (part.includes("-")) {
      const [start, end] = part.split("-").map((h) => parseInt(h, 16));
      for (let i = start; i <= end; i++) {
        result += String.fromCodePoint(i);
      }
    } else if (part.includes("?")) {
      const base = part.replace(/\?/g, "0");
      const top = part.replace(/\?/g, "F");
...
async function main() {
  ...
  for (const font of FONTS) {
    ...
    const outputPath = join(FONTS_DIR, font.outputFile);
  ...
  for (const lic of LICENSE_FILES) {
    ...
    const outputPath = join(FONTS_DIR, lic.outputFile);
